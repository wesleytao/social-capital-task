...
var color = d3.scaleThreshold() //continues to discgrete mapping
    .domain(d3.range(-4, 5))//set color change here
...
function ready(error, us) {
    ...
    svg.append("text")
          ...
          .attr("text-anchor", "start")
          .attr("font-weight", "bold")
          .text("Social Capital Index by County");//if not found then undefined
...
function UpdateData14(){
  ...
  d3.queue()
     .defer(d3.json, "https://d3js.org/us-10m.v1.json")
     .defer(d3.tsv, "../output/rate_2014.tsv", function (d) { social_capital.set(d.id, d.rate); }) //read the data and store in social_capital(set)
     .await(ready);

---
title: "Social_Capital Mediation Analysis"
...
```{r}
m1<-plm(sk14~Gini_Coefficient,data=merged)
summary(m1)
```
...
Multilevel Regression with random effect for state

```{r}
library(lme4)
merged$State<-as.factor(merged$State)
multi_level<-lmer(Gini_Coefficient~State+(State|Jim_Crow),na.action =na.omit ,data=merged)
...
$$Mediate\space Effect=Y_i(t,M_i(1))-Y_i(t,M_i(0)))$$
